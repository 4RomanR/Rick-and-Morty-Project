...
const ResidentInfo = ({ resdURL }) => {
  ...
  return (
    <section className="relative border-2 border-green-500 text-white ">
      ...
      {/* Status */}
      <div className="flex justify-center items-center gap-2 relative bottom-16 w-[120px] left-1/2 -translate-x-1/2 bg-black/60 text-white  border-2 border-green-500">
        <div className={`h-3  aspect-square rounded-full ${status[resdUser?.status]}`}></div>
        <p>{resdUser?.status}</p>
      </div>
      <h3 className="px-4 my-2">Resident Name</h3>
      <ul className="px-2 ">
